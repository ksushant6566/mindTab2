...
export const CreateTask: React.FC<CreateTaskProps> = ({
    ...
    const [formData, setFormData] = React.useState<any>({
        ...
        priority: defaultValues?.priority || TASK_PRIORITY[0],
        impact: defaultValues?.impact || TASK_IMPACT[1],
    ...
    return (
        ...
        >
                    ...
                    <SelectTrigger className="size-8 w-fit focus:ring-0">
                        <SelectValue placeholder="Impact">
                            <span className="flex items-center gap-0 capitalize">
                                {Array.from({ length: impactNumber[formData.impact as keyof typeof impactNumber] }).map((_, i) => (
                    ...
                    <SelectTrigger className="size-8 w-fit focus:ring-0">
                        <SelectValue placeholder="Project">
                            <span className="flex items-center gap-1 text-xs">
                                ...
                                {formData.projectId
                                    ? (projects as any[])?.find((p: any) => p.id === formData.projectId)?.name?.substring(0, 10) || "Project"
                                    : "No Project"}
                            </span>
                        </SelectValue>
                    </SelectTrigger>
                    <SelectContent className="text-sm">
                        <SelectGroup>
                            ...
                            <SelectItem value="none">
                                <span className="flex items-center gap-2 text-xs">No Project</span>
                            </SelectItem>
            ...
            <div className="flex items-center justify-end space-x-2">
                ...
                <Button type="submit" size="sm" className="h-8 text-xs" disabled={loading || !formData.title} loading={loading}>Add task</Button>
            </div>
        </form>
    );
};
