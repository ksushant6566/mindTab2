...
import { TASK_PRIORITY, TASK_IMPACT } from "@mindtab/shared";
...
export const EditTask: React.FC<EditTaskProps> = ({ onSave, onCancel, task, loading = false }) => {
    ...
    const [formData, setFormData] = React.useState<any>({
        ...
        priority: task.priority, impact: task.impact, status: task.status, projectId: task.projectId ?? task.project?.id ?? null,
    });
    ...
    return (
        <form onSubmit={handleSubmit} onKeyDown={handleCmdEnterSubmit} className="flex flex-col gap-2 rounded-lg border p-6">
            <div className="space-y-2">
                <input type="text" id="title" name="title" placeholder="Task name" value={formData.title || ""} onChange={handleChange} required className="w-full bg-inherit text-xl font-semibold focus:border-none focus:outline-none" />
            ...
            <div className="flex gap-2 pb-2">
                <Select onValueChange={(value) => setFormData({ ...formData, priority: value })} value={formData.priority}>
                    ...
                    <SelectContent className="text-sm"><SelectGroup>
                        {TASK_PRIORITY.map((value) => (<SelectItem key={value} value={value}><span className="flex items-center gap-2 capitalize"><Flag className="h-4 w-4" color={priorityColors[value]} fill={priorityColors[value]} />{value.replace("_", " ")}</span></SelectItem>))}
                ...
                <Select onValueChange={(value) => setFormData({ ...formData, impact: value })} value={formData.impact}>
                    ...
                    <SelectContent className="text-sm"><SelectGroup><SelectLabel>Impact</SelectLabel>
                        {[...TASK_IMPACT].reverse().map((value) => (<SelectItem key={value} value={value}><span className="flex items-center gap-0 capitalize"><span className="mr-1 text-sm">{value}</span>{Array.from({ length: impactNumber[value] }).map((_, i) => (<Zap key={i} className="h-3 w-3" color="gold" fill="gold" />))}</span></SelectItem>))}
                    </SelectGroup></SelectContent>
                </Select>
