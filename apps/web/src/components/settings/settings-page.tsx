...
export function SettingsPage() {
  ...
  return (
    <main className="flex h-screen overflow-hidden bg-background text-foreground">
      <aside className="flex w-[292px] shrink-0 flex-col border-r border-border bg-[var(--bg-elev)]/80">
        ...
        <div className="px-3 pb-3">
          <button
            ...
            className="flex h-9 items-center gap-2 rounded-[var(--r-3)] px-2 text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground"
          >
          ...
          <nav className="space-y-1">
            {filteredSections.map((section) => (
              <button
                ...
                className={cn(
                  "flex h-9 w-full items-center gap-3 rounded-[var(--r-3)] px-2 text-left text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground",
                  activeSection === section.id && "bg-secondary text-foreground"
                )}
              >
                {section.icon}
        ...
        <div className="border-t border-border px-4 py-3">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <Text as="div" className="truncate">{user.name || "MindTab user"}</Text>
              <MetaText as="div" className="truncate">{user.email}</MetaText>
            </div>
          </div>
        </div>
      </aside>

      <section className="custom-scrollbar min-w-0 flex-1 overflow-y-auto">
...
function ProfileSettingsSection({
  ...
  return (
    <SettingsPanel title="Profile" description="Your public account basics and lifetime workstation activity.">
      <Card className="overflow-hidden bg-[var(--bg-elev)]/70">
        ...
        <div className="grid grid-cols-3 divide-x divide-border">
          <StatCell label="Lifetime tasks" value={taskCount} />
          <StatCell label="Notes" value={noteCount} />
          <StatCell label="Chats" value={chatCount} />
        </div>
      </Card>
    </SettingsPanel>
  );
    ...
    <SettingsPanel title="Keyboard Shortcuts" description="Current shortcuts are read-only. Customization will come later.">
      <div className="grid gap-4 md:grid-cols-2">
        {shortcutGroups.map((group) => (
          <SettingsCard key={group.label} className="p-0">
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <Text variant="muted" className="mt-2 max-w-2xl">{description}</Text>
      </div>
      {children}
    </div>
  );
...
function SettingsCard({ className, children }: { className?: string; children: ReactNode }) {
  return (
    <Card className={cn("overflow-hidden bg-[var(--bg-elev)]/70", className)}>
...
function ColorControl({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  return (
    <label className="flex h-9 w-[160px] cursor-pointer items-center gap-2 rounded-[var(--r-3)] border border-border bg-background px-2">
      <span className="size-5 rounded-full border border-border" style={{ background: value }} />
...
function RangeControl({
  ...
  return (
    <div className={cn("flex w-[250px] items-center gap-3", disabled && "opacity-45")}>
      <input
        type="range"
        min={min}
        max={max}
...
function Avatar({ user, className }: { user: Pick<User, "name" | "image">; className?: string }) {
  return (
...
function StatCell({ label, value }: { label: string; value: number }) {
  return (
    <div className="p-5">
      <Heading as="div" variant="page">{value.toLocaleString()}</Heading>
      <MetaText as="div" className="mt-1">{label}</MetaText>
    </div>
  );
}
