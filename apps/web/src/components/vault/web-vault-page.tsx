                ...
                <div className="mt-4 flex flex-wrap gap-2">
                    {FILTERS.map((item) => (
                        <button
                            ...
                            className={cn(
                                "h-8 rounded-[var(--r-2)] border px-3 text-[length:var(--type-meta-size)] capitalize",
                                filter === item
                ...
                {isLoading ? (
                    <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
                        {Array.from({ length: 6 }).map((_, index) => (
                            <div key={index} className="h-36 animate-pulse rounded-[var(--r-3)] bg-secondary" />
                        ))}
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
                        {filteredSaves.map((save) => (
                            <Link
                                ...
                                className="group flex min-h-[140px] gap-3 rounded-[var(--r-3)] border border-border bg-background/70 p-3 hover:bg-secondary/50"
                            >
                                ...
                                <div className="min-w-0 flex-1">
                                    <MetaText className="flex items-center gap-2 capitalize">
                                        {sourceIcon(save.source_type)}
                                        {sourceLabel(save.source_type)}
                                        {save.processing_status && <span>{save.processing_status}</span>}
                                    </MetaText>
                                    <Heading as="div" variant="panel" className="mt-2 line-clamp-2">
                                        {save.source_title || save.source_url || "Untitled save"}
                                    </Heading>
                                    {save.summary && (
                                        <Text as="p" variant="muted" className="mt-2 line-clamp-2">{save.summary}</Text>
                                    )}
                                    {save.created_at && (
                                        <MetaText className="mt-2">{format(new Date(save.created_at), "MMM d, yyyy")}</MetaText>
                                    )}
                                </div>
                            </Link>
                        ))}
                    ...
                    <div className="flex h-full min-h-[320px] items-center justify-center text-center">
                        <div>
                            <Heading as="div" variant="panel">No saved items</Heading>
                            <Text as="p" variant="muted" className="mt-2">Your saved articles, media, and notes will appear here.</Text>
                        </div>
                    </div>
                )}
            </div>
            ...
            <div className="custom-scrollbar min-h-0 flex-1 overflow-y-auto p-5">
                {isLoading || !save ? (
                    <div className="space-y-3">
                        <div className="h-56 animate-pulse rounded-[var(--r-3)] bg-secondary" />
                        <div className="h-28 animate-pulse rounded-[var(--r-3)] bg-secondary" />
                    </div>
                ) : (
                    <div className="space-y-5">
                        {(save.media_url || save.source_thumbnail_url || save.video_thumbnail_url) && (
                            <div className="overflow-hidden rounded-[var(--r-3)] border border-border bg-secondary">
                                <img
                        ...
                        {save.summary && (
                            <section className="rounded-[var(--r-3)] border border-border bg-background/70 p-4">
                                <Heading as="h2" variant="panel">Summary</Heading>
                        ...
                        {save.tags && save.tags.length > 0 && (
                            <div className="flex flex-wrap gap-2">
                                {save.tags.map((tag) => (
                                    <MetaText key={tag} className="rounded-[var(--r-2)] border border-border bg-background px-2 py-1">
                                        {tag}
                                    </MetaText>
                                ))}
                            </div>
                        ...
                        {(save.extracted_text || save.visual_description) && (
                            <section className="rounded-[var(--r-3)] border border-border bg-background/70 p-4">
                                <Heading as="h2" variant="panel">
                                    {save.extracted_text ? "Extracted text" : "Visual description"}
                                </Heading>
