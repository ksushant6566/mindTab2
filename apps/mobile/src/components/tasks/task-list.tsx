...
import { tasksQueryOptions } from "@mindtab/core";
...
export function TaskList() {
  const { data: tasks = [], isLoading, isFetching, refetch } = useQuery(tasksQueryOptions(api));
  ...
  const sections = useMemo(() => {
    return statusOrder
      .map((status) => ({
        ...
        data: sortTasks(
          (tasks as Task[]).filter((g) => g.status === status),
          sortBy,
        ),
      }))
